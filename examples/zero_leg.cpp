...
int32_t zeroOffsetCounts(Leg leg, LegSegment segment, uint32_t counts_per_rev) {
    const double magnitude =
        kZeroOffsetDegrees / 360.0 * static_cast<double>(counts_per_rev);
...
bool homeSegment(stablecops::app::MotorDrive& drive, Leg leg, LegSegment segment,
                 const stablecops::ds402::HomingConfig& base_config,
                 uint32_t counts_per_rev) {
    stablecops::ds402::HomingConfig config = base_config;
    config.home_offset = zeroOffsetCounts(leg, segment, counts_per_rev);

    const double offset_deg = static_cast<double>(config.home_offset) /
                              static_cast<double>(counts_per_rev) * 360.0;
    std::cout << "\n=== zeroing " << segmentName(segment) << " drive ===\n"
              << "  zero offset: " << offset_deg << " deg (" << config.home_offset
              << " counts)\n";
    ...
    std::cout << "  homing OK: lower=" << result.lower_limit_position
              << " upper=" << result.upper_limit_position
              << " travel=" << result.travel << " center=" << result.center_position
              << " (offset " << offset_deg << " deg applied)\n";
    return true;
}
...
int main(int argc, char** argv) {
    ...
    stablecops::ds402::HomingConfig homing;  // library defaults
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--leg") {
            ...
            homing.search_velocity = std::stoi(next("--search-velocity"));
        } else if (arg == "--approach-velocity") {
            homing.approach_velocity = std::stoi(next("--approach-velocity"));
        } else if (arg == "--threshold-torque") {
            homing.threshold_torque = static_cast<int16_t>(std::stoi(next("--threshold-torque")));
        } else if (arg == "--backoff-distance") {
            homing.backoff_distance = std::stoi(next("--backoff-distance"));
        } else if (arg == "--timeout-ms") {
            homing.timeout = std::chrono::milliseconds(std::stoi(next("--timeout-ms")));
        } else if (arg == "--no-save") {
            save_to_nvm = false;
        } else if (arg == "--dcf") {
    ...
    base.counts_per_rev = counts_per_rev;

    std::cout << "zeroing " << (leg == Leg::Left ? "left" : "right") << " leg on "
              << can_interface << ", segment(s): " << segment_spec
