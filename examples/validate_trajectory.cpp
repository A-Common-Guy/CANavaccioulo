...
int main(int argc, char** argv) {
    ...
    std::printf("replaying trajectory (%zu samples)...\n", trajectory.size());
    ...
    for (const auto& sample : trajectory) {
        ...
        // Pace to the sample's timestamp relative to replay start.
        const auto target_time =
            t0 + std::chrono::duration_cast<std::chrono::steady_clock::duration>(
                     std::chrono::duration<double>(sample.time));
    ...
    out << "time,motor_1_rad,motor_2_rad,pitch_measured,pitch_expected,roll_measured,roll_expected\n";
    out.setf(std::ios::fixed);
    out.precision(9);
    for (const auto& row : log) {
        out << row.time << ',' << row.motor1_rad << ',' << row.motor2_rad << ',' << row.pitch_meas
